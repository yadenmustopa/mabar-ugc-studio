...
import { Product, Character, GenerationItem, TaskStatus, ObjectStorage } from '../types';
import { showToast, base64ToBlob, urlToBase64 } from '../utils';
import { ASPECT_RATIOS, DEFAULT_MIN_DURATION, URL_UPLOAD_ASSET, RESOLUTIONS } from '../constants';
    ...
    try {
      ...
      for (let i = 0; i < form_data.amount; i++) {
        ...
        try {
          ...
          const video_blob = await aiService.generateVideoVeo(main_scene_b64, storyboard_chunks[0].description, form_data.aspect_ratio);
          
          const local_video_url = URL.createObjectURL(video_blob);
          set_generations_list(prev => prev.map(g => g.id === temp_id ? { ...g, generate_url: local_video_url } : g));
          ...
          const filename = `ugc_${global_ugc_id}_${ugc_item_id}_${Date.now()}.mp4`;
          try {
            const s3_url = await s3Service.uploadVideoToS3(video_blob, target_storage, form_data.bucket, filename);
            await mabarApi.setCompleteItem(global_ugc_id!, ugc_item_id, s3_url);
            set_generations_list(prev => prev.map(g => g.id === temp_id ? { ...g, status: TaskStatus.COMPLETED, progress: 100, generate_url: s3_url } : g));
          } catch (s3_err: any) {
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                    {products_list.map(p => {
                      ...
                      return (
                        <button 
                          key={p.id} 
                          onClick={() => set_form_data({...form_data, product_id: p.id.toString()})}
                          className={`group relative aspect-square rounded-2xl overflow-hidden border-2 transition-all ${is_selected ? 'border-blue-500 ring-4 ring-blue-500/20 shadow-xl' : 'border-white/5 hover:border-white/10'}`}
                        >
                          ...
                          {is_selected && (
                            <div className="absolute top-2 right-2 w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center shadow-lg animate-in zoom-in">
                              <i className="fas fa-check text-[10px] text-white"></i>
                            </div>
                          )}
                          <div className="absolute inset-x-0 bottom-0 bg-slate-900/80 p-2 border-t border-white/5 opacity-0 group-hover:opacity-100 transition-opacity">
                             <p className="text-[7px] font-black text-white uppercase truncate text-center leading-tight">{p.name}</p>
                          </div>
                        </button>
                      );
                    })}
                    ...
                    {characters_list.map(c => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="absolute inset-x-0 bottom-0 bg-slate-900/90 backdrop-blur-sm p-1.5 border-t border-white/5">
                             <p className="text-[7px] font-black text-white uppercase truncate text-center leading-tight">{c.name}</p>
                          </div>
                          {is_selected && (
                            <div className="absolute top-1 right-1 w-4 h-4 bg-blue-600 rounded-full flex items-center justify-center shadow-lg animate-in zoom-in">
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Resolution</label>
                    <select value={form_data.resolution} onChange={e => set_form_data({...form_data, resolution: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl px-5 py-4 text-xs text-white outline-none focus:ring-1 ring-blue-500 transition-all">
                      {RESOLUTIONS.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Aspect Ratio</label>
                    <select value={form_data.aspect_ratio} onChange={e => set_form_data({...form_data, aspect_ratio: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl px-5 py-4 text-xs text-white outline-none focus:ring-1 ring-blue-500 transition-all">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Jumlah Video</label>
                    <input type="number" min="1" max="10" value={form_data.amount} onChange={e => set_form_data({...form_data, amount: parseInt(e.target.value) || 1})} className="w-full bg-slate-950 border border-white/10 rounded-xl px-5 py-4 text-xs text-white outline-none focus:ring-1 ring-blue-500 transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Min Duration (s)</label>
                    <input type="number" min="5" max="60" value={form_data.min_duration} onChange={e => set_form_data({...form_data, min_duration: parseInt(e.target.value) || 5})} className="w-full bg-slate-950 border border-white/10 rounded-xl px-5 py-4 text-xs text-white outline-none focus:ring-1 ring-blue-500 transition-all" />
                  </div>
                </div>
                ...
                <div className="pt-8 border-t border-white/5 space-y-6">
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Storage Server</label>
                        <select value={form_data.object_storage_id} onChange={e => handleStorageChange(e.target.value)} className="w-full bg-slate-950 border border-white/10 rounded-xl px-5 py-4 text-xs text-white outline-none focus:ring-1 ring-blue-500 transition-all">
                          {storages_list.map(s => <option key={s.object_storage_id} value={s.object_storage_id.toString()}>{s.label}</option>)}
                        </select>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Target Bucket</label>
                        <select value={form_data.bucket} onChange={e => set_form_data({...form_data, bucket: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl px-5 py-4 text-xs text-white outline-none focus:ring-1 ring-blue-500 transition-all">
                ...
                <button onClick={startProduction} disabled={is_generating} className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-600 text-white font-black py-6 rounded-[2rem] uppercase tracking-widest text-xs transition-all shadow-2xl shadow-blue-600/20 active:scale-95 flex items-center justify-center space-x-3">
                ...
                generations_list.map((item) => (
                  <div key={item.id} className="bg-slate-950/40 border border-white/5 rounded-3xl p-5 transition-all hover:border-white/10 group relative overflow-hidden">
                    <div className="flex flex-col space-y-4">
                      ...
                      <div className="aspect-video bg-slate-900 rounded-2xl flex items-center justify-center border border-white/5 relative overflow-hidden group/vid">
                        {item.generate_url ? (
                          <video 
                            src={item.generate_url} 
                            className="w-full h-full object-cover" 
                            controls 
                            autoPlay={item.status === TaskStatus.COMPLETED} 
                            muted 
                            loop 
                          />
                        ) : (
                        ...
                        {!item.generate_url && item.status !== TaskStatus.FAILED && (
                      ...
                      {item.failed_reason && (
                        <div className="p-3 bg-rose-500/5 border border-rose-500/10 rounded-xl">
                          <p className="text-[8px] text-rose-400 italic leading-relaxed line-clamp-2">{item.failed_reason}</p>
                        </div>
                      )}
                      ...
                      <div className="flex items-center space-x-2 pt-2">
                        {item.generate_url && (
                           <a 
                            ...
                            download={`Mabar_Studio_Video_${item.order_index}.mp4`}
                            target="_blank" 
                            rel="noopener noreferrer" 
                            className="flex-1 bg-slate-900 hover:bg-slate-800 border border-white/5 py-2.5 rounded-xl text-[8px] font-black text-slate-300 uppercase tracking-widest flex items-center justify-center space-x-2 transition-all"
                           >
